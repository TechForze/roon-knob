...
- `lvgl_weather/` – The `lv_esp_idf` weather demo entrypoint (`dispaly_lvgl_demos_main.c`) that initializes LVGL on an ESP32, starts the display backend, and runs `lv_demo_music()`/`lv_demo_widgets()`, providing a reference for the kind of LVGL polling/layout we need.
